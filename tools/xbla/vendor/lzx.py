...
"""
# Derived from Binary Refinery 0.11.2; see BINARY_REFINERY_LICENSE.md.
from __future__ import annotations

from array import array

from .huffman import BitDecoderBase, HuffmanDecoder, HuffmanDecoder7b, OutOfBounds


def uint32array(length: int):
    """Return a zero-filled native 32-bit unsigned array."""
    result = array('I', [0]) * length
    if result.itemsize != 4:
        raise RuntimeError('This platform has no 32-bit unsigned array type.')
    return result
...
class LzxDecoder:
    ...
    def _decompress(self, cur_size: int | float):
        win = self.window
        bits = self._bits
