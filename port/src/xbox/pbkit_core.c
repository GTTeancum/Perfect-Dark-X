...
// Project-local nxdk core override; see PBKIT_PATCH.md for provenance and fix.
...
volatile unsigned g_SPXBPbkitDeferredSwaps = 0;
...
static void pb_vbl_handler(void)
{
    ...
    {
        // The DPC can observe another VBlank while GPU interrupts are being
        // drained with INTA disabled. In that case ISR did not update scanout.
        // Retire only a frame the ISR actually selected. Otherwise leave it
        // queued for the next ISR, keeping scanout writes at VBlank onset.
        const DWORD next_frame = pb_FBAddr[pb_BackBufferIndex[next]] & 0x03FFFFFF;
#if defined(PD_XBOX_ISSUE3_DIAGNOSTIC)
        // Observe the original queue retirement without changing its behavior.
        if ((VIDEOREG(PCRTC_START) & 0x03FFFFFF) != next_frame)
            ++g_SPXBPbkitDeferredSwaps;
        if (1) {
#else
        if ((VIDEOREG(PCRTC_START) & 0x03FFFFFF) == next_frame) {
#endif
...
#if defined(PD_XBOX_ISSUE3_DIAGNOSTIC)
// Approximate, read-only snapshot: interrupts can advance the queue between reads.
void pdx_issue3_queue_snapshot(unsigned *s)
{
    s[0]=pb_vbl_counter; s[1]=pb_back_index;
    s[2]=pb_BackBufferNxt; s[3]=pb_BackBufferNxtVBL;
    for (int i=0;i<3;i++) {
        s[4+i]=pb_BackBufferbReady[i]; s[7+i]=pb_BackBufferIndex[i];
        s[10+i]=pb_FBAddr[i];
    }
}
#endif
